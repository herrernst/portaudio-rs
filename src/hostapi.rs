...
impl HostApiInfo
{
    fn from_ll(input: &ll::PaHostApiInfo) -> HostApiInfo
    {
        HostApiInfo
        {
            ...
            name: String::from_utf8_lossy(unsafe { CStr::from_ptr(input.name as *const u8).to_bytes() }).into_owned(),
...
impl HostErrorInfo
{
    fn from_ll(input: &ll::PaHostErrorInfo) -> HostErrorInfo
    {
        HostErrorInfo
        {
            ...
            text: String::from_utf8_lossy(unsafe { CStr::from_ptr(input.errorText as *const u8).to_bytes() }).into_owned(),
